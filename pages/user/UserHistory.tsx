        ...
        <div className="flex flex-wrap items-center gap-2 mb-6">
          ...
          {['TODOS', 'PENDENTE', 'APROVADO', 'REPROVADO', 'EXPIRADO'].map((filter) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-[10px] font-bold tracking-wide transition-all border ${activeFilter === filter
                  ...
                  {filteredRecords.map((record) => (
                    <React.Fragment key={record.id}>
                      <tr
                        ...
                        className={`cursor-pointer transition-colors group ${expandedRecordId === record.id ? 'bg-blue-50/50' : 'hover:bg-blue-50/30'
                          }`}
                      >

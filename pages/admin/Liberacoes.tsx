...
const AdminLiberacoes = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // Validar input do RAI (apenas números, máx 8)
    if (name === 'rai') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      await supabase.from('system_logs').insert({
        level: 'INFO',
        action: 'UPDATE_LIBERACAO',
        details: { rai: formData.rai, changes: formData },
        matricula: 'ADMIN',
      ...
      await supabase.from('system_logs').insert({
        level: 'INFO',
        action: 'CREATE_LIBERACAO',
        details: { rai: formData.rai, data: formData },
        matricula: 'ADMIN',
        ip_address: '127.0.0.1'
      });
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Deseja revogar esta liberação permanentemente?")) {
      const item = liberacoes.find(l => l.id === id);
      ...
      if (item) {
        await supabase.from('system_logs').insert({
          level: 'WARN',
          action: 'REVOKE_LIBERACAO',
          details: { rai: item.rai, id: item.id },
          matricula: 'ADMIN',
          ip_address: '127.0.0.1'
        });
      }
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredList.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-4 py-3 text-center font-black text-slate-700">{item.rai.padStart(8, '0')}</td>

...
const SETTINGS_MENUS = [
    ...
    { id: 'LOGS', label: 'Logs do Sistema', icon: 'history', subtitle: 'Histórico de Atividades' },
        ...
        <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Últimas 50 ações realizadas no sistema.</p>
                </div>
                <button
                    onClick={fetchLogs}
                    disabled={loading}
            ...
            <div className="overflow-hidden border border-slate-200 rounded-xl">
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            <th className="px-4 py-3 font-bold text-slate-600">Data/Hora</th>
                            <th className="px-4 py-3 font-bold text-slate-600">Ação</th>
                            <th className="px-4 py-3 font-bold text-slate-600">Usuário</th>
                            <th className="px-4 py-3 font-bold text-slate-600 text-right">Detalhes</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {logs.map((log) => (
                            <tr key={log.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-4 py-3">
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${log.level === 'ERROR' ? 'bg-red-100 text-red-600' :
                                            log.level === 'WARN' ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'
                                        }`}>
                                        {log.action}
                                    </span>
                                </td>
                                <td className="px-4 py-3 font-bold text-slate-700">{log.matricula || 'Sistema'}</td>
                                <td className="px-4 py-3 text-slate-500 truncate max-w-[200px] text-right" title={JSON.stringify(log.details)}>
                        ...
                        {logs.length === 0 && !loading && (
                            <tr>
                                <td colSpan={4} className="px-4 py-8 text-center text-slate-400 italic">Nenhum log encontrado.</td>
                            </tr>
                        )}
                    </tbody>
                </table>

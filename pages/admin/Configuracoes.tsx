...
const SETTINGS_MENUS = [
    ...
    { id: 'LOGS', label: 'Logs do Sistema', icon: 'history', subtitle: 'Auditoria e Rastreabilidade' },
...
const AdminConfiguracoes = () => {
    ...
    const handleSave = async () => {
        ...
        await supabase.from('system_logs').insert({
            ...
            details: { config },
            matricula: 'ADMIN', // Idealmente pegar do contexto de auth
                    ...
                    <div className="p-2 space-y-1 flex-1 overflow-y-auto">
                        {SETTINGS_MENUS.map((menu) => (
                            <button
                                ...
                                className={`w-full text-left p-4 rounded-xl transition-all group relative overflow-hidden ${activeMenu === menu.id
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4">
                                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${activeMenu === menu.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500 group-hover:text-slate-700'
                                        }`}>
                    ...
                    {activeMenu === 'LOGS' && (
                        <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
                            <div className="border-b border-slate-100 pb-4 mb-2">
                                <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                                    <span className="material-icons-round text-slate-600">history</span>
                                    Logs de Auditoria
                                </h2>
                                <p className="text-sm text-slate-500 mt-1">Histórico completo de ações realizadas no sistema.</p>
                            </div>
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Últimos 50 Registros</h3>
                <button onClick={fetchLogs} className="text-blue-600 hover:text-blue-800 text-xs font-bold flex items-center gap-1">
            ...
            {loading ? (
                <div className="p-8 text-center text-slate-400">
                    ...
                    <p className="text-xs mt-2">Carregando logs...</p>
                </div>
            ) : logs.length > 0 ? (
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {logs.map((log) => (
                                <tr key={log.id} className="hover:bg-slate-50">
                                    <td className="px-4 py-3 font-mono text-slate-500 whitespace-nowrap">{formatDate(log.created_at)}</td>
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex flex-col">
                                            <span className="text-slate-800 font-medium">{log.matricula || 'Sistema'}</span>
                                            <span className="text-[9px] text-slate-400">{log.ip_address || '-'}</span>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3 text-slate-500 max-w-xs truncate" title={JSON.stringify(log.details)}>
